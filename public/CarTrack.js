...
T.CarTrack = function (map, opt) {
  ...
  this.options.polylinestyle = this.setOptions(
...
T.CarTrack.prototype = {
  ...
  init: function () {
    ...
    this.D3OverLayer = new T.D3Overlay(
      this.d3init,
      this.d3redraw,
      this.options
    );
  ...
  setOptions: function (obj, options) {
    for (var i in options) {
      if (i != "polylinestyle" && i != "carstyle") obj[i] = options[i];
    }
    return obj;
  },
  /**
  ...
  receiveData: function () {
    ...
    if (opt.Datas instanceof Array && opt.Datas.length > 0) {
      ...
      //绘制车辆
      me.carMarker = new CarOverlay(
        me.dataToLnglat(this.options.Datas[0]),
        me.options.carstyle
      );
      if (!this.options.carstyle) me.carMarker.hide();
  ...
  dataToLnglat: function (obj) {
    if (obj instanceof T.LngLat || ("lat" in obj && "lng" in obj)) return obj;
    else {
  ...
  d3init: function (sel, transform) {
    ...
    sel
      ...
      .attr(
        "display",
        this.options.dynamicLine && this.options.polylinestyle.display
          ? "block"
          : "none"
      );
  },
  ...
  d3redraw: function () {
    ...
    var datasStr1 = pointsToPath(
      [lnglatsTopoints(this.map, this.options.Datas)],
      false
    ); //this._svg.
    var lineDatas = this.lineDatas
      ? this.lineDatas
    ...
    d3.select("path#polyline" + this.options.uid)
      .attr("d", datasStr1)
      .attr("stroke-width", this.options.polylinestyle.width + "px");

    d3.select("path#dynamicLine" + this.options.uid)
      .attr("d", datasStr2)
      .attr("stroke-width", this.options.polylinestyle.width + "px");
  },
  ...
  update: function () {
    ...
    var linePath = d3
      .select("path#polyline" + this.options.uid)
      .attr(
        "display",
        this.options.polylinestyle.display && !this.options.dynamicLine
          ? "block"
          : "none"
      );
    //轨迹像素长度
    ...
    if (this.options.speed > 0) {
      ...
      this.D3OverLayer.lineDatas = this.options.Datas.slice(
        0,
        this.options.Counter
      );
    }

    this.carMarker.setLnglat(
